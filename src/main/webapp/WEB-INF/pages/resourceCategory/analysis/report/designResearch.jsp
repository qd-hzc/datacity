...
<script type="text/javascript">
    ...
    //报表类型
    var rptDesignType = RPT_DESIGN_TYPE.CUSTOM;
    //表样信息
    var style = ${style};
    //    模板信息
    ...
    //单位选择器
    var tdUnitSelector = {};
    var MARGIN_LEFT = '0 0 0 20';

    var TIME_RANGE = timeRanges.length > 0 ? timeRanges[0].type : 1;
    var period = research.period ? research.period : 1;
    var createHtmlTable = style.rptStyle;
    //渲染页面
    Ext.onReady(function () {
        ...
        var groupStore = Ext.create('Ext.data.TreeStore', {
            ...
            proxy: {
                type: 'ajax',
                api: {
                    read: contextPath + '/resourcecategory/analysis/common/analysis/getGroupInfoTrees'
                }
            },
            root: {
                text: '根节点',
        ...
        var mainBarStore = Ext.create('Ext.data.TreeStore', {
            ...
            proxy: {
                type: 'ajax',
                api: {
                    read: contextPath + '/resourcecategory/analysis/report/designCustomResearch/getReportBarTree'
                },
                extraParams: {
                    isMainBar: 1,
                    styleId: research.id
        ...
        var nodeInfoStore = new Ext.data.Store({
            fields: ['dataName', 'dataValue', 'dataInfo1', 'dataInfo2', 'dataType', 'isRealNode', 'isProperty'],
            data: []
        });
        //节点信息表格菜单
            ...
            tbar: ['<b>表样</b>', '-', {
                ...
                handler: function () {
                    ...
                    var fo = {
                        ...
                        lianxuBeginYearCombobox: lianxuBeginYearCombobox,
                        lianxuBeginPeriodCombobox: lianxuBeginPeriodCombobox,
                        lianxuEndYearCombobox: lianxuEndYearCombobox,
                        lianxuEndPeriodCombobox: lianxuEndPeriodCombobox,
                        xuanzeYearMultyCombobox: xuanzeYearMultyCombobox,
                        xuanzePeriodMultyCombobox: xuanzePeriodMultyCombobox,
                        baogaoqi: baogaoqishuTextField,
                        researchId: research.id
                    };
                    RESEARCHHZC.saveRangeAndFrequency(fo, function (rs) {
                        if (rs.success) {
                            RESEARCHHZC.genTableStr(mainBarTree, guestBarTree, fo, function (data) {
                                if (data.success) {
                                    initTdClickHandler();
                ...
                handler: function () {
                    if (!createHtmlTable) {
                        Ext.Msg.alert('提示', '请先生成表样，并保存');
                        return;
                    }
                    window.open(contextPath + '/resourcecategory/analysis/report/designCustomResearch/showPreviewResearch?researchId=' + research.id);
                }
            }, '-', {
                xtype: 'button',
                text: '',
                ...
                handler: function () {
                    ...
                    RESEARCHHZC.saveRangeAndFrequency(fo, function (rs) {
                        if (rs.success) {
                            RESEARCHHZC.saveBarInfos(fo, mainBarTree, guestBarTree, function (data) {
                                if (data.success) {
...
//                            hzc
        ...
        var yearStore = Ext.create('Ext.data.Store', {
            ...
            proxy: {
                type: 'ajax',
                url: contextPath + '/resourcecategory/analysis/report/designCustomResearch/getYearStore'
            },
            autoLoad: true
        });
        var quarterStore = Ext.create('Ext.data.Store', {
            fields: ['id', 'name'],
            data: [['3', '一季度'], ['6', '二季度'], ['9', '三季度'], ['12', '四季度']]
        });
        var halfYearStore = Ext.create('Ext.data.Store', {
            fields: ['id', 'name'],
            data: [['6', '上半年'], ['12', '下半年']]
        });
...
//        时间范围：连续：开始年
        var lianxuBeginYearCombobox = Ext.create('Ext.form.ComboBox', {
            name: 'beginYear',
            store: yearStore,
            valueField: 'code',
            displayField: 'name',
            ...
            listeners: {
                change: function (_this, newValue, oldValue, eOpts) {
                    var endYear = lianxuEndYearCombobox.getValue();
                    if (endYear && parseInt(endYear) > 0) {
                        if (parseInt(newValue) > parseInt(endYear)) {
                            Ext.Msg.alert('提示', '开始年不能大于结束年');
...
//        时间范围：连续：结束年
        var lianxuEndYearCombobox = Ext.create('Ext.form.ComboBox', {
            name: 'endYear',
            store: yearStore,
            valueField: 'code',
            displayField: 'name',
            ...
            listeners: {
                change: function (_this, newValue, oldValue, eOpts) {
                    var beginYear = lianxuBeginYearCombobox.getValue();
                    if (beginYear && parseInt(beginYear) > 0) {
                        if (parseInt(newValue) < parseInt(beginYear)) {
                            Ext.Msg.alert('提示', '开始年不能大于结束年');
                            _this.setValue(oldValue);
                        }
                    }
...
//        时间范围：连续：开始频度
        var lianxuBeginPeriodCombobox = Ext.create('Ext.form.ComboBox', {
            name: 'beginPeriod',
            store: monthStore,
            valueField: 'id',
            displayField: 'name',
...
//        时间范围：选择：结束频度
        var xuanzePeriodMultyCombobox = Ext.create('Ext.form.field.ComboBox', {
            name: 'endPeriod',
            store: monthStore,
            valueField: 'id',
            displayField: 'name',
...
//        时间范围：报告期数
        var baogaoqishuTextField = Ext.create('Ext.form.TextField', {
            name: 'periods',
            value: '1',
            width: 100,
            margin: '0 0 0 27'
...
//        时间范围：连续
        var lianxufanwei = Ext.create('Ext.form.FieldSet', {
            border: 0,
            layout: 'hbox',
            items: [{
                xtype: 'radiofield',
                ...
                handler: function (checkbox, checked) {
                    if (checked) {
                        TIME_RANGE = 1;
                        lianxuBeginYearCombobox.setDisabled(false);
                        lianxuEndYearCombobox.setDisabled(false);
                        if (period != 1) {
                            lianxuBeginPeriodCombobox.setDisabled(false);
                            lianxuEndPeriodCombobox.setDisabled(false);
                        } else {
                            lianxuBeginPeriodCombobox.setDisabled(true);
                            lianxuEndPeriodCombobox.setDisabled(true);
                        }
                        baogaoqishuTextField.setDisabled(true);
                        xuanzeYearMultyCombobox.setDisabled(true);
                        xuanzePeriodMultyCombobox.setDisabled(true);
                    }
                }
            }, {
                xtype: 'displayfield',
                value: '从',
                margin: MARGIN_LEFT
            }, lianxuBeginYearCombobox,
                lianxuBeginPeriodCombobox, {
                    xtype: 'displayfield',
                    value: '到',
                    margin: MARGIN_LEFT
                }, lianxuEndYearCombobox,
                lianxuEndPeriodCombobox]
        });
//        时间范围：选择
        var xuanzefanwei = Ext.create('Ext.form.FieldSet', {
            border: 0,
            layout: 'hbox',
            items: [{
                xtype: 'radiofield',
                ...
                handler: function (checkbox, checked) {
                    if (checked) {
                        TIME_RANGE = 2;
                        lianxuBeginYearCombobox.setDisabled(true);
                        lianxuBeginPeriodCombobox.setDisabled(true);
                        lianxuEndYearCombobox.setDisabled(true);
                        lianxuEndPeriodCombobox.setDisabled(true);
                        baogaoqishuTextField.setDisabled(true);
                        xuanzeYearMultyCombobox.setDisabled(false);
                        if (period != 1) {
                            xuanzePeriodMultyCombobox.setDisabled(false);
                        } else {
                            xuanzePeriodMultyCombobox.setDisabled(true);
                        }
                    }
                ...
                xuanzeYearMultyCombobox,
                xuanzePeriodMultyCombobox]
        });
        var timeRange = Ext.create('Ext.form.RadioGroup', {
            fieldLabel: '时间范围',
            labelWidth: 70,
            labelAlign: 'right',
            width: '100%',
                    ...
                    items: [{
                        ...
                        handler: function (checkbox, checked) {
                            if (checked) {
                                ...
                                lianxuEndYearCombobox.setDisabled(true);
                                lianxuEndPeriodCombobox.setDisabled(true);
                                baogaoqishuTextField.setDisabled(false);
                                xuanzeYearMultyCombobox.setDisabled(true);
                                xuanzePeriodMultyCombobox.setDisabled(true);
                            }
                        }
                    }, baogaoqishuTextField]
                }
            ],
            listeners: {
                afterrender: function () {
                    _setFrequence(period);
                    switch (TIME_RANGE) {
                        case 1:
                            lianxuBeginYearCombobox.setDisabled(false);
                            lianxuEndYearCombobox.setDisabled(false);
                            ...
                            xuanzeYearMultyCombobox.setDisabled(true);
                            xuanzePeriodMultyCombobox.setDisabled(true);
                            Ext.getCmp('timeRange1').setValue(true);
                            Ext.Array.each(timeRanges, function (t) {
                                var number = parseInt(t.dataType);
                                if (number == METADATA_TYPE.DATA_BEGIN_YEAR) {
                                    lianxuBeginYearCombobox.setValue(t.dataValue);
                                } else if (number == METADATA_TYPE.DATA_END_YEAR) {
                                    lianxuEndYearCombobox.setValue(t.dataValue);
                                } else if (number == METADATA_TYPE.DATA_BEGIN_PERIOD) {
                                    lianxuBeginPeriodCombobox.setValue(t.dataValue);
                                } else if (number == METADATA_TYPE.DATA_END_PERIOD) {
                                    lianxuEndPeriodCombobox.setValue(t.dataValue);
                                }
                            });
                            break;
                        case 2 :
                            ...
                            Ext.getCmp('timeRange2').setValue(true);
                            var yearList = [];
                            var periodList = [];
                            Ext.Array.each(timeRanges, function (t) {
                                if (parseInt(t.dataType) == METADATA_TYPE.DATA_YEAR) {
                                    yearList.push(t.dataValue);
                                } else {
                                    periodList.push(t.dataValue);
                                }
                            });
                            xuanzeYearMultyCombobox.setValue(yearList);
                            xuanzePeriodMultyCombobox.setValue(periodList);
                            break;
                        case 3 :
                            lianxuBeginYearCombobox.setDisabled(true);
                            lianxuBeginPeriodCombobox.setDisabled(true);
                            ...
                            xuanzeYearMultyCombobox.setDisabled(true);
                            xuanzePeriodMultyCombobox.setDisabled(true);
                            Ext.getCmp('timeRange3').setValue(true);
                            baogaoqishuTextField.setValue(timeRanges[0].dataValue);
                            break;
                    }
                }
            }
...
//        设置时间范围
        function _setFrequence(frequencyId) {
            var abc = parseInt(frequencyId);
            switch (abc) {
                case 1:
                    lianxuBeginPeriodCombobox.setDisabled(true);
                    lianxuEndPeriodCombobox.setDisabled(true);
                ...
                case 2:
                    ...
                    lianxuBeginPeriodCombobox.setStore(halfYearStore);
                    lianxuEndPeriodCombobox.setStore(halfYearStore);
                    xuanzePeriodMultyCombobox.setStore(halfYearStore);
                    period = 2;
                    break;
                case 3:
                    if (TIME_RANGE == 1) {
                    ...
                    lianxuBeginPeriodCombobox.setStore(quarterStore);
                    lianxuEndPeriodCombobox.setStore(quarterStore);
                    xuanzePeriodMultyCombobox.setStore(quarterStore);
                    period = 3;
                    break;
                case 4:
                    if (TIME_RANGE == 1) {
                    ...
                    lianxuBeginPeriodCombobox.setStore(monthStore);
                    lianxuEndPeriodCombobox.setStore(monthStore);
                    xuanzePeriodMultyCombobox.setStore(monthStore);
                    period = 4;
                    break;
            }
        }

        var timefrequency = Ext.create('Ext.form.RadioGroup', {
            fieldLabel: '时间频度',
            labelWidth: 70,
            labelAlign: 'right',
            layout: 'hbox',
...
//                        scrollable: true,
            ...
            listeners: {
                ...
                afterrender: function () {
//                    beginYearCombobox.setStore(monthStore);
                }
            }
        });
        //容器
...
//        时间控制方法
        function timeHandler(record, overModel) {
            showAddTimeWindow(record, overModel.get('root'), function (target, recordNew, formData) {
                if (target == 'same') {
        ...
        function checkAnotherTreeValid(thisNode, anotherTree) {
            ...
            var props = [];
            var leaves = [];
            RESEARCHHZC.getTreeNodes(anotherTree.getRootNode(), null, leaves);
            if (leaves.length) {
...
<script type="text/javascript" charset="utf-8"
        src="<%=contextPath%>/City/support/regime/report/design/addNodeInfoWin.js"></script>
<script type="text/javascript" charset="utf-8" src="<%=contextPath%>/City/common/queryTreeByLocal.js"></script>
<script type="text/javascript" charset="utf-8"
        src="<%=contextPath%>/City/support/regime/report/design/addItemWin.js"></script>
<script type="text/javascript" charset="utf-8"
        src="<%=contextPath%>/City/support/regime/report/design/dragGroupWin.js"></script>
<script type="text/javascript" charset="utf-8"
        src="<%=contextPath%>/City/support/regime/report/design/addTimeFrameWin.js"></script>
<script type="text/javascript" charset="utf-8"
        src="<%=contextPath%>/City/support/regime/report/design/copyNodeWin.js"></script>
...
<script type="text/javascript" charset="utf-8"
        src="<%=contextPath%>/City/support/regime/report/ueditor.config.js"></script>
<script type="text/javascript" charset="utf-8"
        src="<%=contextPath%>/City/resourceCategory/analysis/report/design/researchStyle.js"></script>
<script type="text/javascript" charset="utf-8"
        src="<%=contextPath%>/City/resourceCategory/analysis/report/design/addTimeWin.js"></script>
<script type="text/javascript" charset="utf-8"
        src="<%=contextPath%>/City/resourceCategory/analysis/report/design/design-function.js"></script>
</body>
</html>
